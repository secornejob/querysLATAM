CREATE OR REPLACE PROCEDURE `bc-te-dlake-dev-s7b3.cln_etracking_us.etracking_repair`()
BEGIN

CREATE OR REPLACE TABLE `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_rep_ship_in` AS
WITH 
...
,SHIPMENT_INBOUND AS (
    ...
    WHERE 
            PH.PO_TYPE_CD = 'REPAIR'
        AND EVT_P.EVENT_STATUS_CD <> 'POCANCEL'
        AND SS.SHIPMENT_TYPE_CD = 'REPAIR'
...
CREATE OR REPLACE TABLE `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_rep_ship_out` AS
WITH 
...
,SHIPMENT_OUTBOUND AS (
    ...
    WHERE 
            PH.PO_TYPE_CD = 'REPAIR'
        AND EVT_P.EVENT_STATUS_CD <> 'POCANCEL'
        AND SS.SHIPMENT_TYPE_CD = 'SENDREP'
        AND EVT_S.EVENT_STATUS_CD <> 'IXCANCEL'
)
...
CREATE OR REPLACE TABLE `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_repair` AS
...
,ROUTING AS (
    SELECT 
        ...
        FROM 
                (   
                SELECT PO_ID,PO_DB_ID,SHIPMENT_ID,SHIPMENT_DB_ID FROM `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_rep_ship_in`
                UNION DISTINCT 
                SELECT PO_ID,PO_DB_ID,SHIPMENT_ID,SHIPMENT_DB_ID FROM `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_rep_ship_out`
                ) AS SHIP
...
from`bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_rep_ship_in` AS SI
...
,FINAL AS(
    SELECT
        ...
        (CASE 
         ...
         -- WHEN (PH.PO_ID IS NOT NULL AND (EVT_P.EVENT_STATUS_CD = 'PORECEIVED' OR EVT_P.EVENT_STATUS_CD = 'POCLOSED') AND SIN.SHIPMENT_INBOUND_COMPLETE IS NOT NULL AND SIN.INV_COND_CD = 'SCRAP')   THEN 'PO SCRAP'
        ...
        R_LLEGADA_F_IN.LLEGADA_STATUS AS IN_LLEGADA_STATUS,
        R_LLEGADA_F_IN.LLEGADA_COMPLETE_HUB_DT AS IN_LLEGADA_COMPLETE_HUB_DT,
        R_SALIDA_F_IN.SALIDA_STATUS AS IN_SALIDA_STATUS ,
        R_SALIDA_F_IN.SALIDA_COMPLETE_HUB_DT AS IN_SALIDA_COMPLETE_HUB_DT,

        R_LLEGADA_F_OUT.LLEGADA_STATUS AS OUT_LLEGADA_STATUS,
        R_LLEGADA_F_OUT.LLEGADA_COMPLETE_HUB_DT AS OUT_LLEGADA_COMPLETE_HUB_DT,
        R_SALIDA_F_OUT.SALIDA_STATUS AS OUT_SALIDA_STATUS ,
        R_SALIDA_F_OUT.SALIDA_COMPLETE_HUB_DT AS OUT_SALIDA_COMPLETE_HUB_DT,
...
--      REMOCION Y TRANSFER
    ...
    LEFT JOIN `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_rep_ship_in` AS SIN   
    ...
    LEFT JOIN `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_rep_ship_out` AS SOUT   
...
DROP TABLE IF EXISTS `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_rep_ship_in`;

DROP TABLE IF EXISTS `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_rep_ship_out`;


CREATE OR REPLACE TABLE `bc-te-dlake-dev-s7b3.cln_etracking_us.repair` AS
WITH tmp_repair AS (
SELECT
       OUTBOUND_STATUS              AS repair_outbound_status_code,
       OUTBOUND_SHIPMENT            AS shipment_outbound_code,
       --
       INBOUND_STATUS               AS repair_inbound_status_code,
       INBOUND_SHIPMENT             AS shipment_inbound_code,
       --
       PO_NUMBER                    AS po_code,
       EVENT_STATUS_CD              AS po_global_status,
       --
     --LLEGADA_STATUS               AS complete_hub_arrival_status_code,
     --SALIDA_STATUS                AS complete_hub_departure_status_code,
       --
       SHIPMENT_OUTBOUND_STATUS     AS shipment_outbound_status_code,
       SHIPMENT_INBOUND_STATUS      AS shipment_inbound_status_code,       
       SHIPMENT_TYPE_CD             AS shipment_inbound_type_code,
       --
       INV_COND_CD                  AS inventory_status_code,
     ...
     --CONTACT_USERNAME             AS contact_user_name,
       ...
       SERIAL_NO_OEM                AS serial_oem_code,
    ...
    -- RE_SHIP_TO_ID                AS ship_to_location_code,
       ...
       OUT_LLEGADA_STATUS           AS outbound_complete_hub_arrival_status_code,
       OUT_SALIDA_STATUS            AS outbound_complete_hub_departure_status_code,
       CAST(OUT_LLEGADA_COMPLETE_HUB_DT AS DATETIME) AS outbound_complete_hub_arrival_dt,
       CAST(OUT_SALIDA_COMPLETE_HUB_DT  AS DATETIME) AS outbound_complete_hub_departure_dt,
       --
       SHIPMENT_INBOUND_COMPLETE    AS shipment_inbound_complete_dt,
       IN_LLEGADA_STATUS            AS inbound_complete_hub_arrival_status_code,
       IN_SALIDA_STATUS             AS inbound_complete_hub_departure_status_code,
       CAST(IN_LLEGADA_COMPLETE_HUB_DT AS DATETIME)  AS inbound_complete_hub_arrival_dt,
       CAST(IN_SALIDA_COMPLETE_HUB_DT  AS DATETIME)  AS inbound_complete_hub_departure_dt,
       --
     --CAST(LLEGADA_COMPLETE_HUB_DT AS DATETIME) AS complete_hub_arrival_dt,
     --CAST(SALIDA_COMPLETE_HUB_DT  AS DATETIME) AS complete_hub_departure_dt,

       RECEIVED_DT                  AS repair_received_dt,
       --
    -- current_datetime(null)       AS REMOVAL_DT,
     ...
     --PO_LAST_MOD_DT               AS po_last_mod_dt,
...
FROM `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_repair`),
--
PO_FECHAS AS (
SELECT  *,
        --
        ...
        -- REMOCION A TRANSFER
        ROUND((DATETIME_DIFF(
                              IFNULL (TRANSFER_CREATION_DT, IF (repair_outbound_status_code='PENDIENTE'
                                                               ,CURRENT_DATETIME()
                                                               ,REMOVAL_DT))
                             ,IFNULL (REMOVAL_DT, IFNULL (TRANSFER_CREATION_DT, IF (repair_outbound_status_code='PENDIENTE'
                                                                                   ,CURRENT_DATETIME()
                                                                                   ,REMOVAL_DT)))
                             ,MINUTE) / 60
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL(TRANSFER_COMPLETE_DT,    IF (repair_outbound_status_code='EN TRANSITO'
                                                                 ,CURRENT_DATETIME()
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL(OUTBOUND_COMPLETE_HUB_ARRIVAL_DT,  IF (repair_outbound_status_code IN ('EN TRANSITO', 'EN HUB', 'EN TRANSIT DESDE HUB HACIA DESTINO')
                                                                           ,CURRENT_DATETIME()
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL(OUTBOUND_COMPLETE_HUB_DEPARTURE_DT,  IF (repair_outbound_status_code IN ('EN TRANSITO', 'EN HUB', 'EN TRANSIT DESDE HUB HACIA DESTINO')
                                                                             ,CURRENT_DATETIME()
                                                                             ,IFNULL (OUTBOUND_COMPLETE_HUB_ARRIVAL_DT, IFNULL(TRANSFER_COMPLETE_DT, IFNULL (TRANSFER_CREATION_DT, REMOVAL_DT)))))
                             ,IFNULL (OUTBOUND_COMPLETE_HUB_ARRIVAL_DT, IFNULL(DELIVERY_DISPATCH_DT, IFNULL (TRANSFER_COMPLETE_DT, REMOVAL_DT)))
                             ,MINUTE) / 60
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL(SHIPMENT_OUTBOUND_COMPLETE_DT, IF (repair_outbound_status_code='COMPLETED'
                                                                       ,CURRENT_DATETIME()
        ...
        --,ROUND((DATETIME_DIFF(
        --                      IFNULL(SHIPMENT_OUTBOUND_COMPLETE_DT,  IF (repair_outbound_status_code= 'PENDIENTE' -- 'PO SEND SHIPMENT'
        --                                                                ,CURRENT_DATETIME()
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL (PO_FIRST_AUTH_DT,      IF (repair_inbound_status_code IN ('PO OPEN', 'PO OPEN - REQUESTED') --'PO AUTORIZADA'
                                                                ,CURRENT_DATETIME()
                                                                ,PO_CREATION_DT))
                             ,PO_CREATION_DT
                             ,MINUTE) / 60
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL(PO_FIRST_ISSUE_DT,      IF (repair_inbound_status_code='PO AUTORIZADA' --''PO ISSUED'
                                                                ,CURRENT_DATETIME()
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL(INBOUND_COMPLETE_HUB_ARRIVAL_DT,    IF (repair_inbound_status_code='PO ISSUED' --'PO IN HUB'
                                                                            ,CURRENT_DATETIME()
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL(INBOUND_COMPLETE_HUB_DEPARTURE_DT,  IF (repair_inbound_status_code = 'PO IN HUB'
                                                                            ,CURRENT_DATETIME()
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL(SHIPMENT_INBOUND_COMPLETE_DT, IF (repair_inbound_status_code = 'PO HUB HACIA DESTINO'
                                                                       ,CURRENT_DATETIME()
        ...
        ,ROUND((DATETIME_DIFF(
                              IFNULL (REPAIR_RECEIVED_DT, IF (repair_inbound_status_code IN ('PO RECEIVED', 'PRE RECEPCION', 'PO QUAR', 'PO PARTIAL') 
                                                             ,CURRENT_DATETIME()
        ...
        ,REPAIR_RECEIVED_DT                 STAGE_REPAIR_RECEIVED_DT
        --
        ,CASE repair_outbound_status_code
                              ...
                              stage_outbound_code
        --
        ,CASE repair_inbound_status_code
                              WHEN 'PO OPEN'                         THEN 1
...
SELECT   *,
       ...
       ,(SELECT SUM(gs.sla_val) FROM `bc-te-dlake-dev-s7b3.cln_etracking_us.general_sla`gs WHERE gs.process_id = 3 AND gs.stage_id IN (1,2,3,4,5,6)    AND gs.req_priority_code = f.req_priority_code) sla_outbound_standard_val
       ,(SELECT SUM(gs.sla_val) FROM `bc-te-dlake-dev-s7b3.cln_etracking_us.general_sla`gs WHERE gs.process_id = 3 AND gs.stage_id IN (7,8,9,10,11,12) AND gs.req_priority_code = f.req_priority_code) sla_inbound_standard_val
         --
         -- FLAG SLA
        ...
        ,IF (f.removal_transfer_val        > (SELECT gs.sla_val FROM `bc-te-dlake-dev-s7b3.cln_etracking_us.general_sla` gs
                                             WHERE  gs.process_id = 3  AND gs.req_priority_code = f.req_priority_code
                                             AND    gs.stage_id = 1
        ...
        ,IF    ((f.removal_transfer_val +
                 ...
                 f.outbound_complete_hub_departure_shipment_outbound_complete_val)
                                                                    > (SELECT SUM(sla_val)
                                                                       ...
                                                                       WHERE gs.process_id = 3
                                                                       AND   gs.req_priority_code = f.req_priority_code
                                                                       AND   (CASE WHEN f.repair_outbound_status_code = 'PENDIENTE'     THEN stage_id IN (1,2,3,4)
                                                                                   WHEN f.repair_outbound_status_code = 'EN TRANSITO'   THEN stage_id IN (5)
                                                                                   WHEN f.repair_outbound_status_code = 'EN HUB'        THEN stage_id IN (5)
                                                                                   WHEN f.repair_outbound_status_code = 'EN TRANSIT DESDE HUB HACIA DESTINO' THEN stage_id IN (5)
                                                                                   WHEN f.repair_outbound_status_code = 'COMPLETED'     THEN stage_id IN (6)
        ...
        ,IF    (f.creation_first_auth_val                              +
                ...
                f.shipment_inbound_complete_rfi_val
                                                                    > (SELECT SUM(sla_val)
                                                                       ...
                                                                       WHERE gs.process_id = 3
                                                                       AND   gs.req_priority_code = f.req_priority_code
                                                                       AND   (CASE WHEN f.repair_inbound_status_code in ('PO OPEN', 'PO OPEN - REQUESTED') THEN stage_id IN (7)
                                                                                   WHEN f.repair_inbound_status_code = 'PO AUTORIZADA'                     THEN stage_id IN (7,8)
                                                                                   WHEN f.repair_inbound_status_code = 'PO ISSUED'                         THEN stage_id IN (7,8,9)
                                                                                   WHEN f.repair_inbound_status_code = 'PO IN HUB'                         THEN stage_id IN (7,8,9,10)
                                                                                   WHEN f.repair_inbound_status_code = 'PO HUB HACIA DESTINO'              THEN stage_id IN (7,8,9,10)
                                                                                   WHEN f.repair_inbound_status_code IN ('PO RECEIVED', 'PRE RECEPCION', 'PO PARTIAL')   THEN stage_id IN (7,8,9,10,11)
                                                                                   WHEN f.repair_inbound_status_code IN ('RFI', 'PO QUAR','OTRO')          THEN stage_id IN (7,8,9,10,11,12)
...
SELECT F.*
     ...
     ,CASE stage_inbound_code
         ...
         WHEN 7  THEN STAGE_REPAIR_RECEIVED_DT
         WHEN 8  THEN STAGE_REPAIR_RECEIVED_DT
         WHEN 9  THEN STAGE_REPAIR_RECEIVED_DT
         WHEN 10 THEN STAGE_REPAIR_RECEIVED_DT
         WHEN 11 THEN STAGE_REPAIR_RECEIVED_DT
...
DROP TABLE IF EXISTS `bc-te-dlake-dev-s7b3.cln_etracking_us.tmp_repair`;
